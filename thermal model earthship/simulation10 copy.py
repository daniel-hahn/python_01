...
import xarray as xr
import pandas as pd
from config10 import *  # Import all constants and variables
import pvlib
...
for t in range(1, total_steps):
    ...
    # Heat loss through walls, floor, roof, windows (through conduction, convection, radiation)
...
plt.figure(figsize=(10, 5))
plt.plot(time_data[:total_steps], Q_solar_direct_useful_plot, label="Solar Gains (W)", color="orange")
plt.plot(time_data[:total_steps], transmission_heat_loss_plot, label="Transmission Heat Loss (W)", color="blue", linestyle="dashed")
plt.xlabel("Time")
plt.ylabel("Heat (W)")
plt.title("Solar Gains vs Heat Losses")
plt.legend()
plt.grid(True)
plt.xticks(rotation=45)
plt.show(block=False)
